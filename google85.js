// google85.js
console.log("google85.js chargé avec succès.");
...
function populateAndPrint(printTitle) {
    // Étape 1 : Préparation de tout le contenu à imprimer
    ...
    if (currentDiveInfo.incident) {
        ...
        let totalConsoIncident = 0;
        const timeStep = 0.1;
        for (let t = 0; t <= incidentTime; t += timeStep) {
            totalConsoIncident += (20 * (1 + getDepthAtTime(t, mainPlan) / 10) * timeStep) / mainInfo.bloc;
        }
        for (let t_remontee = 0; t_remontee < incidentPlan.dtrFinale; t_remontee += timeStep) {
            totalConsoIncident += (20 * (1 + getDepthAtTime(t_remontee, incidentPlan) / 10) * timeStep * 2) / mainInfo.bloc;
        }
        const pressionFinaleIncident = 200 - totalConsoIncident;
        document.getElementById('print-panne-air').textContent = `${incidentTime} min`;
        document.getElementById('print-incident-total-time').textContent = `${(incidentTime + incidentPlan.dtrFinale).toFixed(1)} min`;
    ...
    if (currentDiveInfo.limits && (currentDiveInfo.limits.reserveDtr || currentDiveInfo.limits.emptyDtr)) {
        ...
        if (currentDiveInfo.limits.reserveDtr) {
            ...
            printReserveWarning.textContent = `Réserve de ${reserveValue} bars entamée dès ${currentDiveInfo.limits.reserveDtr} min de DTR.`;
        }
        if (currentDiveInfo.limits.emptyDtr) {
            printEmptyWarning.textContent = `Risque de panne sèche dès ${currentDiveInfo.limits.emptyDtr} min de DTR.`;
        }
    ...
    // Étape 2 : LA CORRECTION ROBUSTE
    // On demande au navigateur de lancer l'impression juste avant son prochain
    // cycle de "redessinage". Ceci garantit que tous les changements (le titre)
    // sont bien visibles sur tous les appareils, même les plus lents.
    requestAnimationFrame(() => {
        // Un très court délai est parfois encore nécessaire en plus pour certains navigateurs
        setTimeout(() => {
            window.print();
        }, 300); 
    });
...
function calculateConsumptionLimits(profondeur, bloc, reserve) {
    if (!diveTableData || !diveTableData.table[profondeur]) {
        return { reserveDtr: null, emptyDtr: null };
    }
    ...
    for (const de of desInTable) {
        ...
        const dp = dt + de;
        const tempPlan = createPlan(profondeur, dp, dtr);

        // On va utiliser une méthode de calcul de conso simple et directe
        ...
        const totalConsumption = consoDescente + consoFond + consoRemontee;
        
        const remainingPressure = 200 - totalConsumption;

        if (remainingPressure < reserve && reserveDtr === null) {
            reserveDtr = dtr;
        }
        if (remainingPressure < 0 && emptyDtr === null) {
            emptyDtr = dtr;
            break; // Pas la peine d'aller plus loin, on a trouvé la panne sèche
        }
    }

    return { reserveDtr, emptyDtr };
}
...
function displayConsumptionWarnings(limits, reserve) {
    ...
    if (!limits || (!limits.reserveDtr && !limits.emptyDtr)) {
